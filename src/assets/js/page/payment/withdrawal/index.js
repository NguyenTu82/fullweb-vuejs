...
import ConfirmWithdrawal from "@/views/payment/withdrawal/components/confirm";
...
export default {
  ...
  watch: {
    amoutInput() {
      this.amoutInput = this.amoutInput || 0;
      this.isButtonDisabled =
        this.amoutInput == 0 || !this.amoutInput ? true : false;
    },
    setSelectAmount() {
      if (this.setSelectAmount === 1) {
  ...
  methods: {
    ...
    inputHandle(event) {
      ...
      this.amoutInput = parseInt(event.target.value);
    ...
    increaseAmount() {
      if (typeof this.amoutInput == "string") {
        if (
          ...
          const parseNumber =
            parseInt(this.amoutInput.replaceAll(",", "")) || 0;
          const newAmountMax = parseNumber - (parseNumber % 100) + 100;
          if (newAmountMax > this.withdrawInfo.WITHDRAW_AVAILABLE_AMOUNT) {
            this.amoutInput = this.withdrawInfo.WITHDRAW_AVAILABLE_AMOUNT;
          } else {
            this.amoutInput = parseNumber - (parseNumber % 100) + 100;
          }
        ...
        if (this.amoutInput < this.withdrawInfo.WITHDRAW_AVAILABLE_AMOUNT) {
          const newAmountMax = this.amoutInput - (this.amoutInput % 100) + 100;
          if (newAmountMax > this.withdrawInfo.WITHDRAW_AVAILABLE_AMOUNT) {
            this.amoutInput = this.withdrawInfo.WITHDRAW_AVAILABLE_AMOUNT;
          } else {
            this.amoutInput = this.amoutInput - (this.amoutInput % 100) + 100;
          }
          this.disabledIncrease = false;
          this.disabledReduce = false;
          if (
    ...
    reducAmount() {
      if (typeof this.amoutInput == "string") {
        const parseNumber = parseInt(this.amoutInput.replaceAll(",", ""));
        this.amoutInput =
          parseNumber % 100 !== 0
            ? parseNumber - (parseNumber % 100)
            : parseNumber - 100;
        ...
        if (this.amoutInput > 100) {
          this.amoutInput =
            parseInt(this.amoutInput) % 100 !== 0
              ? parseInt(this.amoutInput) - (parseInt(this.amoutInput) % 100)
              : parseInt(this.amoutInput) - 100;
          this.disabledIncrease = true;
          if (parseInt(this.amoutInput) == 0) {
            this.disabledReduce = true;
    ...
    changeSelectAmount(mount) {
      this.setSelectAmount = mount;
    },
  },
};
